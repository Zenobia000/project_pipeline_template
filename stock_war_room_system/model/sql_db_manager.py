...
if __name__ == "__main__":
    ...
    with PostgreSQLManager(config) as postgres_manager:
        postgres_manager.create_table(
            "employees", {"id": "SERIAL PRIMARY KEY", "name": "TEXT", "age": "INT"}
        )

        new_employee_id = postgres_manager.insert_data(
            "employees", {"name": "Bob", "age": 25}
        )
        logging.info(f"Inserted PostgreSQL employee ID: {new_employee_id}")

        employee = postgres_manager.read_data(
            "employees", conditions={"id": new_employee_id}
        )
        logging.info(f"Queried PostgreSQL employee: {employee}")

        postgres_manager.update_data("employees", {"age": 26}, {"id": new_employee_id})
        updated_employee = postgres_manager.read_data(
            "employees", conditions={"id": new_employee_id}
        )
